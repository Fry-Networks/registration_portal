...
import IopoolModal from '../components/modals/water/Iopool';
import EcowittModal from '../components/modals/water/Ecowitt';
...
const WaterPortal = () => {
  ...
  const handleIopool = async (apiKey: string): Promise<boolean> => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_HOST}/api/submitIopool`,
        {
          method: 'POST',
          headers: { 'Content-type': 'application/json' },
          body: JSON.stringify({
      ...
      if (!onlyPortal) {
        router.push({
          pathname: '/register',
          query: { minerKey, type: 'iopool' }
        });
      } else {
        if (portalType === undefined) {
          await setPortalType(minerKey, 'iopool');
        }
        router.push('/devices');
      }
    } catch (error) {
