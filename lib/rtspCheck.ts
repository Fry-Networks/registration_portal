...
export async function checkRtspLink(rtspUrl: string, opts?: { timeoutMs?: number }): Promise<RtspCheckResult> {
  const timeoutMs = opts?.timeoutMs ?? 5000;

  return new Promise((resolve) => {
    let parsed: url.UrlWithStringQuery;
    try {
      parsed = url.parse(rtspUrl);
    } catch (e: any) {
    ...
    socket.on('error', (err: any) => {
      ...
      resolve({ ok: false, code, message: String(err?.message || err), host, port });
    });
