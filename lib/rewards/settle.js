...
function planSettle(doc, opts) {
    ...
    if (custodial) {
        ...
        for (let i = 0; i < records.length; i++) {
            ...
            const idx = arr.findIndex((/** @type {any} */ e, /** @type {number} */ j) => !taken[source].has(j) && e && e.status === CLAIMABLE && sameElement(e, sel[i]));
            if (idx < 0) { issue('ROW_CHANGED', describe(sel[i], source, r.amount)); continue; }
