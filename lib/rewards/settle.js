 ...
 * and writes each element positionally (weekly_rewards.<i> / daily_rewards.<i>), guarded by that element's stored
 * identity at that index (_id + week_start|date + status [+ claiming_group]). Anything it cannot resolve to exactly one
...
/**
 * The identity pin of exactly one element (matched in JS, and mapped by positionalGuard onto arr.<i>.<field>
 * filter keys for the positional write): values are taken from the stored element as
