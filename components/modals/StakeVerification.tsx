...
export default function StakeVerification({
  ...
  const [FRYamount, setFRYAmount] = useState<{
    stake_one: number;
    stake_two: number;
  ...
  useEffect(() => {
    const fetchMinerTypes = async () => {
      try {
        ...
        let stake_data = data.data.stake as {
          stake_one: number;
          stake_two: number;
        };
      ...
      <DialogPanel className="max-w-xl w-full mx-auto p-4 md:p-6 relative bg-white dark:bg-dark-tremor-background rounded-lg shadow-lg">
        ...
        <form>
          ...
          <p className="text-tremor-content-subtle dark:text-dark-tremor-content-subtle">
            All $FRY sent will be locked for 24h OR 6 months before you can
            withdraw them again.
          </p>
          ...
          <div className="flex flex-col md:flex-row gap-4 mt-4">
            ...
            >
              {isLoading
                ? 'Processing...'
                : `Stake (${FRYamount.stake_one} $FRY) 24h Lock`}
            </Button>
            ...
            >
              {isLoading
                ? 'Processing...'
                : `Stake (${FRYamount.stake_two} $FRY) 6 months Lock`}
            </Button>
          </div>
        </form>
      </DialogPanel>
