...
interface IopoolModalProps {
  modalName: string;
  minerKey: string | string[] | undefined;
  handle: (apiKey: string) => Promise<boolean>;
}

const IopoolModal: React.FC<IopoolModalProps> = ({
  modalName,
  minerKey,
  handle
}: IopoolModalProps) => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_HOST}/api/getDeviceCredential`, {
          ...
          body: JSON.stringify({ miner_key: minerKey, type: 'iopool' })
        });
  ...
  return (
    ...
    >
      <DialogPanel className="sm:max-w-2xl">
        ...
        <Title className="mb-5">IO Pool</Title>
        ...
        <TextInput
          ...
          error={apiKey !== "" && !/\b^[a-zA-Z0-9]{40}$\b/gm.test(apiKey)}
        ...
        >
          ...
          <Button
            ...
            `}
            disabled={!/\b^[a-zA-Z0-9]{40}$\b/gm.test(apiKey)}
            onClick={() => {
              handleSubmit();
            }}
          >
