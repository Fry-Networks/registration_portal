...
interface GmcMapModalProps {
  modalName: string;
  minerKey: string | string[] | undefined;
  handle: (paramID: string) => Promise<boolean>;
}

const GmcMapModal: React.FC<GmcMapModalProps> = ({
  modalName,
  minerKey,
  handle
}: GmcMapModalProps) => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_HOST}/api/getDeviceCredential`, {
          ...
          body: JSON.stringify({ miner_key: minerKey, type: 'gmcmap' })
        });
  
        const result = await response.json();
        if (result.data !== null) {
          setApiKey(result.data.paramID);
        }
      } catch (error) {
        console.error(error);
        return;
  ...
  return (
    ...
    >
      <DialogPanel className="sm:max-w-2xl">
        ...
        <Title className="mb-5">GMC Map API</Title>
        <Subtitle className='mb-2 text-[14px]'>
          Please enter your GMC Credentials below:
        </Subtitle>
        <TextInput
          type="text"
          value={apiKey}
          ...
          error={apiKey !== "" && !/\b^[0-9]{11}$\b/gm.test(apiKey)}
        ...
        >
          ...
          <Button
            ...
            `}
            disabled={!/\b^[0-9]{11}$\b/gm.test(apiKey)}
            onClick={() => {
              handleSubmit();
            }}
          >
